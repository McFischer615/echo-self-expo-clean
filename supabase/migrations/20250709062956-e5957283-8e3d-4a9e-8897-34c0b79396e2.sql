...
CREATE TABLE public.webhook_deliveries (
  ...
  webhook_event_uuid TEXT NOT NULL REFERENCES public.webhook_events(uuid) ON DELETE CASCADE,
...
-- Add rate_limit_tier_id column to oauth_applications
ALTER TABLE public.oauth_applications
ADD COLUMN IF NOT EXISTS rate_limit_tier_id UUID REFERENCES public.rate_limit_tiers(id);

-- Update existing applications to use free tier
UPDATE public.oauth_applications 
SET rate_limit_tier_id = (SELECT id FROM public.rate_limit_tiers WHERE name = 'free' LIMIT 1)
WHERE rate_limit_tier_id IS NULL;
